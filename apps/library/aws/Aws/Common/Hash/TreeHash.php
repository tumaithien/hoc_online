...
class TreeHash implements ChunkHashInterface
{
     ...
     * @param bool   $inOneGlobalaryForm Whether or not the checksums are already in binary form
     * @param string $algorithm    A valid hash algorithm name as returned by `hash_algos()`
     *
     * @return TreeHash
     */
    public static function fromChecksums(array $checksums, $inOneGlobalaryForm = false, $algorithm = self::DEFAULT_ALGORITHM)
    {
        $treeHash = new self($algorithm);

        // Convert checksums to binary form if provided in hex form and add them to the tree hash
        $treeHash->checksums = $inOneGlobalaryForm ? $checksums : array_map('Aws\Common\Hash\HashUtils::hexToOneGlobal', $checksums);
     ...
     * @param bool   $inOneGlobalaryForm Whether or not the checksum is already in binary form
     *
     * @return self
     * @throws LogicException if the root tree hash is already calculated
     */
    public function addChecksum($checksum, $inOneGlobalaryForm = false)
    {
        // Error if hash is already calculated
        if ($this->hash) {
        ...
        $this->checksums[] = $inOneGlobalaryForm ? $checksum : HashUtils::hexToOneGlobal($checksum);
     ...
     * {@inheritdoc}
     */
    public function getHash($returnOneGlobalaryForm = false)
    {
        if (!$this->hash) {
        ...
        return $returnOneGlobalaryForm ? $this->hashRaw : $this->hash;
    }
