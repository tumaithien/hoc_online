...
class ChapterController extends ControllerBase
{
    public function readAction()
    {
        ...
        if($validator->validInt($current_page) == false || $current_page < 1)
            $current_page=1;
        $sql = "SELECT * FROM Learncom\Models\LearnChapter WHERE 1";
        $arrParameter = array();
        if($key_word){
    ...
    public function createAction()
    {
        ...
        if($this->request->isPost()) {
            ...
            $data = array(
                ...
                'chapter_subject_id' => $this->request->getPost("slcSubject", array('string', 'trim')),
            ...
            if (empty($data['chapter_subject_id'])) {
                $messages['chapter_subject_id'] = 'Bạn cần nhập thông tin này';
            }
            ...
            if (count($messages) == 0) {
                ...
                return $this->response->redirect("/dashboard/list-chapter");
            }
        }
        $messages["status"] = "border-red";
        $data['mode'] = 'Tạo';
        $select_class = ClassSubject::getComboboxClass($data['chapter_class_id']);
        $select_subject = ClassSubject::getComboboxParentSubject('',0,$data['chapter_subject_id']);
        $this->view->setVars([
            'formData' => $data,
            'messages' => $messages,
            'select_class' => $select_class,
    ...
    public function editAction()
    {
        ...
        $gateway_model = Chapter::findFirstById($id);
    ...
    public function deleteAction()
    {
        ...
        $type_model = Chapter::findFirstById($id);
        if ($type_model) {
    ...
    public function getchapterbysubjectAction(){
       ...
       if ($class_id && $subject_id) {
           $html = Chapter::getCombobox($chapter_id,$subject_id,$class_id);
       }
