...
class VideolessonController extends ControllerBase
{
    public function readAction()
    {
        ...
        if($validator->validInt($current_page) == false || $current_page < 1)
            $current_page=1;
        $sql = "SELECT * FROM Learncom\Models\LearnVideo WHERE video_type='baigiang'";
        $arrParameter = array();
        if($key_word){
        ...
        if ($chapter) {
            $sql.= " AND video_chapter_id = :chapter:";
    ...
    public function createAction()
    {
        ...
        if($this->request->isPost()) {
            ...
            $data = array(
                ...
                'video_chapter_id' => $this->request->getPost("slcChapter", array('string', 'trim')),
            );
            ...
            if (count($messages) == 0) {
                ...
                return $this->response->redirect("/dashboard/list-videolesson");
            }
        }
    ...
    public function editAction()
    {
        ...
        if($this->request->isPost()) {
            ...
            if (empty($data['video_type'])) {
                $messages['video_type'] = 'Bạn cần nhập thông tin này';
            }
            if (empty($data['video_name'])) {
                $messages['video_name'] = 'Bạn cần nhập thông tin này';
            }
