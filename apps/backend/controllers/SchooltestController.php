...
class SchooltestController extends ControllerBase
{
    public function readAction()
    {
        ...
        if($validator->validInt($current_page) == false || $current_page < 1)
            $current_page=1;
        $sql = "SELECT * FROM Learncom\Models\LearnTest WHERE test_parent_id = 0 AND test_type = 'new' ";
        $arrParameter = array();
        if($key_word){
    ...
    public function createAction()
    {
        ...
        if($this->request->isPost()) {
            ...
            if (count($messages) == 0) {
                ...
                return $this->response->redirect("/office/list-new-test");
            }
        }
    ...
    public function editAction()
    {
        ...
        if($this->request->isPost()) {
            ...
            $data = array(
                ...
                'test_score_choose' => $this->request->getPost("txtScroreChoose", array('string', 'trim')),
                'test_time_start' => strtotime($this->request->getPost('txtStartTime')) - 7*3600,
                'test_status' => $this->request->getPost('radStatus'),
            );
...
//                    }
            ...
            foreach ($items_checked as $id) {
                ...
                if ($test_model) {
                    if ($test_model->delete() === false) {
                        $message_delete = 'Can\'t delete the student name = ' . $test_model->getTestName();
                        $msg_result['status'] = 'error';
    ...
    public function editdetailAction() {
        ...
        if (!$test_model) {
            return $this->response->redirect('/dashboard/notfound');
        }
